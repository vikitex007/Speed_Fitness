...
import TrainerAvatar from '../assets/cbum.png'; // Use your trainer image asset
import { Crown, User, UserCog, LogOut, Utensils, Dumbbell, CheckCircle, Phone, MessageCircle } from 'lucide-react';
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col items-center py-8 min-h-screen shadow-lg">
        ...
        <nav className="flex flex-col gap-2 w-full px-4">
          <a href="/dashboard" className="flex items-center gap-3 font-semibold text-lg text-gray-600 py-3 px-4 rounded-xl hover:bg-gray-50 hover:text-gray-900 transition-all">
            <Crown className="w-6 h-6" /> Dashboard
          </a>
          ...
          <div className="w-full max-w-4xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-8 mb-10">
              <div className="bg-white rounded-2xl shadow-md flex flex-col items-center p-6 w-full md:w-1/3 border border-gray-100">
                ...
                <div className="font-bold text-xl text-gray-900 mb-1">{trainer.name}</div>
                <div className="text-sm text-gray-500 mb-2">Personal Trainer</div>
                <div className="flex flex-col gap-1 text-gray-700 text-sm w-full">
                  <div><span className="font-semibold">Experience:</span> {trainer.experience}</div>
                  <div><span className="font-semibold">Achievements:</span> {trainer.achievements}</div>
                </div>
            ...
            {/* Chat Modal */}
            <TrainerChatModal open={showChat} onClose={() => setShowChat(false)} trainer={trainer} user={user} />
          </div>
        </main>
      </div>
    </div>
