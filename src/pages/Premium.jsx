...
const Premium = () => {
  return (
    <div className="min-h-screen bg-black py-10 px-2 md:px-8">
      <h1 className="text-4xl font-bold text-center text-red-500 mb-10">Premium Service</h1>
      <div className="flex flex-col gap-10">
        {packages.map((pkg, idx) => (
          <div
            key={pkg.name}
            className={`flex flex-col md:flex-row items-stretch rounded-2xl shadow-xl ${pkg.bg} max-w-4xl mx-auto min-h-[420px] md:h-[420px] w-full`}
          >
            {/* Left: Image */}
            <div className="md:w-64 w-full flex justify-center items-center p-8 md:p-0 bg-black/10">
              <img
                src={pkg.image}
                alt={pkg.name}
            ...
            {/* Right: Details */}
            <div className="flex-1 flex flex-col justify-between p-6 md:p-10 text-white h-full">
              ...
              <div className="flex items-center gap-6 flex-wrap mt-2 md:mt-4">
                ...
                <button className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-md font-semibold shadow transition-colors">Upgrade</button>
              </div>
            </div>
          </div>
        ))}
